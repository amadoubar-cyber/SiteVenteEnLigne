...
const ConstructionMaterials = () => {
  ...
  const { data: productsData, isLoading } = useQuery(
    ['construction-products', { page, search, category, sort, order, brand }],
    async () => {
      try {
  ...
  const ProductCard = ({ product, viewMode }) => (
    <div className={`product-card card ${viewMode === 'list' ? 'flex flex-row p-4' : 'p-4'}`}>
      ...
      <div className={`space-y-2 ${viewMode === 'list' ? 'ml-4 flex-1' : ''}`}>
        ...
        {/* Informations spécifiques aux matériaux de construction */}
          ...
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              ...
              <p className="text-secondary-600">
                Tout pour vos projets de construction et rénovation
              </p>
            </div>
          </div>
          <p className="text-secondary-600">
